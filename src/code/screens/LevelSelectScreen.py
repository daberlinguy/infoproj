...
class LevelSelectScreen(Screen):
    def __init__(self, screen, caption, world_id):
        ...
        self.prev_rect = pygame.Rect(grid_x, grid_y + self.levels_per_page * row_height + 10, 140, 44)
        self.next_rect = pygame.Rect(grid_x + row_width - 140, grid_y + self.levels_per_page * row_height + 10, 140, 44)
    ...
    def run(self):
        ...
        self.draw_text(title, getFont(48), 255, 255, 255,
                       (self.screen.get_width() / 2) - 200, (self.screen.get_height() / 2) - 260)

        if not self.levels:
            self.draw_text("No levels found for this world", getFont(24), 255, 255, 255,
                           (self.screen.get_width() / 2) - 220, (self.screen.get_height() / 2) - 20)
        else:
            mouse_pos = pygame.mouse.get_pos()
            levels_page = self._get_page_levels()
            progress = SETTINGS.get("progress", {}).get("worlds", {}).get(self.world_id, {}).get("levels", {})
            for idx, row_rect in enumerate(self.row_rects):
                if idx >= len(levels_page):
                    continue
                ...
                border_color = (40, 200, 40) if is_completed else ((140, 140, 140) if is_hovered else (90, 90, 90))
                ...
                self.draw_text(level["name"], getFont(24), 255, 255, 255, row_rect.x + 20, row_rect.y + 18)

            total_pages = max(1, (len(self.levels) - 1) // self.levels_per_page + 1)
            page_label = f"{self.page + 1} / {total_pages}"
            self.draw_text(page_label, getFont(20), 255, 255, 255,
                           (self.screen.get_width() / 2) - 30, self.prev_rect.y + 8)

            pygame.draw.rect(self.screen, (50, 50, 50), self.prev_rect, border_radius=6)
            pygame.draw.rect(self.screen, (120, 120, 120), self.prev_rect, 2, border_radius=6)
            self.draw_text("Prev", getFont(20), 255, 255, 255, self.prev_rect.x + 36, self.prev_rect.y + 8)

            pygame.draw.rect(self.screen, (50, 50, 50), self.next_rect, border_radius=6)
            pygame.draw.rect(self.screen, (120, 120, 120), self.next_rect, 2, border_radius=6)
            self.draw_text("Next", getFont(20), 255, 255, 255, self.next_rect.x + 36, self.next_rect.y + 8)
